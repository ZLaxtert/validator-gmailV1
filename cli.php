...
enterlist:
echo "\n$WH [$GR+$WH] Your file ($YL example.txt $WH) $GR>> $BL";
...
foreach ($lists as $list) {
    ...
    $api = $APIs . "/validator/gmail/?list=$email&proxy=$Proxies&proxyAuth=$proxy_Auth&type_proxy=$type_proxy&apikey=$apikey";
    // CURL
    $ch = curl_init();
    ...
    if (strpos($x, '"status":"live"')) {
        ...
        echo "[$RD$no$DEF/$GR$total$DEF]$CY RECHECK$DEF =>$BL $list$DEF | [$YL MSG$DEF:$MG TOO MANY REQUESTS, PLEASE CHECK YOUR PROXY$DEF ] | BY$CY DARKXCODE$DEF (DEMO)" . PHP_EOL;
        ...
        echo "[$RD$no$DEF/$GR$total$DEF]$RD DIE$DEF =>$BL $list$DEF | [$YL MSG$DEF:$MG BAD REQUESTS$DEF ] | BY$CY DARKXCODE$DEF (DEMO)" . PHP_EOL;
    } else if ($x == '') {
        $die++;
        save_file("result/bad-req.txt", "$list");
        echo "[$RD$no$DEF/$GR$total$DEF]$RD DIE$DEF =>$BL $list$DEF | [$YL MSG$DEF:$MG ERROR$DEF ] | BY$CY DARKXCODE$DEF (DEMO)" . PHP_EOL;
    } else {
        $unknown++;
